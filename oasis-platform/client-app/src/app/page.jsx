...
function HibiscusPetal({ angle }) {
  return (
    <group rotation={[0, 0, angle]}>
      <mesh position={[0.6, 0, 0]} rotation={[0, 0.3, 0]} scale={[1, 0.5, 0.1]}>
  ...
  return (
    <Float speed={2} rotationIntensity={2} floatIntensity={2}>
      <group scale={[0.8, 0.8, 0.8]}>
        {[0, 1, 2, 3, 4].map((i) => (
          <HibiscusPetal key={i} angle={(i / 5) * Math.PI * 2} />
        ))}
        <mesh position={[0, 0, 0.8]} rotation={[Math.PI / 2, 0, 0]}>
          <cylinderGeometry args={[0.04, 0.06, 1.6, 16]} />
          <meshPhysicalMaterial color="#ffebb3" metalness={0.1} roughness={0.4} />
        </mesh>
        <mesh position={[0, 0, 1.6]}>
          <sphereGeometry args={[0.12, 16, 16]} />
          <meshPhysicalMaterial color="#ff4081" clearcoat={1} roughness={0.1} />
        </mesh>
      </group>
    </Float>
  );
